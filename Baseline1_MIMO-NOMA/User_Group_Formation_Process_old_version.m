...
%% Value assignment and Renew Group Idx
for group_idx = 1 : BS.num_of_SCs
    group_ = Groups(group_idx).user_subset;
    len_group_ = length(find(group_ ~= 0));
    if len_group_ == 1
        %% if this UE occpuies this SC alone
        canadidate_group = inf*ones(BS.num_of_SCs, 1);
        ue_single_idx =  group_(1);
        ...
        if Min_value < inf
            Groups(group_idx).user_subset = 0;
