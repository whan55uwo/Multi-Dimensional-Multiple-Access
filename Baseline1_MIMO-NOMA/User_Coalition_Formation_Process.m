function [] = User_Coalition_Formation_Process()
...
for u = 1 : BS.num_of_UE
    Users(u).delta_beta = 0.8*(1/BS.pow_allocate_coeff - 3+sqrt((10 - 1/BS.pow_allocate_coeff)^2 + 4*BS.Nt*rho*(Users(u).dist^(-3.76))))/...
        (2*BS.Nt*rho*(Users(u).dist^(-3.76)));
    %% Power-domain NOMA
    if Users(u).if_SIC_capability == 1
        Users(u).delta_d = 10^((3 + 37.6*log10(Users(u).dist/1000))/37.6)*1000 - Users(u).dist;
...
while 1
    ...
    %% Break Condition for outer iteration
...
for group_idx = 1 : BS.num_of_SCs
    ...
    for pair_idx = 1:3
        ...
        if ue_weak_idx == 0
            ...
            Users(ue_weak_idx).beam_vector = Users(ue_strong_idx).beam_vector;
            Users(ue_weak_idx).if_S_NOMA = (~isempty(other_ue_set));
            Users(ue_weak_idx).partner_S_NOMA = other_ue_set;
        end
        if ue_weak_idx == 0
            [cost_UE] = g_cost_cal(ue_strong_idx);
        else
            [cost_UE] = g_cost_cal(ue_strong_idx);
            [cost_UE] = g_cost_cal(ue_weak_idx);
        end
