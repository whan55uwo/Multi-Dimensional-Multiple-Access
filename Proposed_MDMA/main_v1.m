...
BS.num_of_UE   = 20; % the number of users in the cell, To Be Detemined (TBD)
...
Num_iteration_alg2_record = zeros(drop_num, 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf('%% **************    Simulation Start!!! 1~%d drops.   ******************* %%\n', drop_num);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for loop_drop = 101:drop_num
    ...
    [sum_cost_est, utilization_cost_est_UE_array, iter_num_ag1]  = User_Coalition_Formation_Process(); % output:UE coalitions
    Num_iteration_alg1_record(loop_drop) = iter_num_ag1;
    fprintf('loop_drop = %d, user coaltion formation. \n', loop_drop);
    %% 6. SC allocation and power allocation
    [iter_num_ag2] = SC_power_allocation();
    Num_iteration_alg2_record(loop_drop) = iter_num_ag2;
    %% 9. Data Saving
    fprintf('loop_drop = %d, Finsih all and Save data!\n', loop_drop);
    savefile = sprintf('../Data/IMDMA/Case_%d_File_idx_%d_Result.mat', CASE_IDX, loop_drop);
    if loop_drop == drop_num
        save(savefile, 'BS', 'Users', 'Beamspaces', 'Coalitions','Num_iteration_alg1_record','Num_iteration_alg2_record');
    else
       save(savefile, 'BS', 'Users', 'Beamspaces', 'Coalitions');
    end
